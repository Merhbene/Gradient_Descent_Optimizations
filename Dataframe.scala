...
val df = DF.repartition(10)
var Dataframe =df.withColumn("partitionID", spark_partition_id)
Dataframe.show()
...
//SGD
def SGD( Df:org.apache.spark.sql.Dataset[org.apache.spark.sql.Row] , eta:Double , W:ArrayBuffer[Double] )={
  var new_W  = W
  var L=Df.select($"x1",$"x2",$"y").collect()
  for (i<-L)
...
for (i<-1 to epoch)
{
  for(i<- 0 to num_partition-1)
  {
    ...
    val curr_Df=Dataframe.filter($"partitionID" === i )
    val nww=SGD( curr_Df , eta , nw )
    nw=nww
    println(nw)
  }
}
...
def SGD_miniBatch( Df:org.apache.spark.sql.Dataset[org.apache.spark.sql.Row] , eta:Double , W:ArrayBuffer[Double] )={
  var new_W  = W
  ...
  gradient=prod_by_scal(gradient,1/(Df.count.toFloat))
...
for (j<-1 to epoch)
 {
  ...
  for(i<- 0 to mini_batch_rate-1){
    ...
    val curr_Df=Dataframe.filter($"partitionID" === i )
    val nww=SGD_miniBatch( curr_Df , eta , nw )
    nw=nww
    println(nw)
  }
}
...
//Momentum
def Momentum( Df:org.apache.spark.sql.Dataset[org.apache.spark.sql.Row] , eta:Double, Gamma:Double, V:ArrayBuffer[Double], W:ArrayBuffer[Double] )=
{
  var new_W  = W
  var new_V=V
...
for (j<-1 to epoch)
{
    ...
    for(i<- 0 to 9)
  {
      ...
      val curr_Df=Dataframe.filter($"partitionID" === i )
      val (nww,nwV)=Momentum( curr_Df , eta ,Gamma , new_V , nw )
      nw=nww
      new_V=nwV
      println(nw)
   }
...
def Adagrad( Df:org.apache.spark.sql.Dataset[org.apache.spark.sql.Row] , eta:Double,  W:ArrayBuffer[Double] )=
{
  var new_W  = W
  var gradient = new ArrayBuffer[Double](W.length)
...
for (j<-1 to epoch)
{
  for(i<- 0 to 9)
  {
      ...
      val curr_Df=Dataframe.filter($"partitionID" === i )
      val nww=Adagrad( curr_Df , eta , nw )
      nw=nww
      println(nw)
  }
